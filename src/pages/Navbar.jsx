...
function Navbar() {
  ...
  useEffect(() => {
    ...
    const fetchUserType = async () => {
      try {
        const response = await fetch("https://arjun-college-project-backend.onrender.com/userType", {
          method: "GET",
  ...
  return (
    <>
      <header className="shadow bg-white fixed w-full z-10 top-0">
        <div className="relative flex max-w-screen-xl-sm-xg flex-col overflow-hidden px-4 py-4 md:mx-auto md:flex-row bg-red-200 md:items-center">
          <a href="/" className="flex items-center whitespace-nowrap text-2xl font-black">
            <span className="text-black"><button className="rounded-md border-2 border-blue-600 px-6 py-1 font-medium text-blue-600 transition-colors hover:bg-blue-600 hover:text-white">ACT GRIEVANCE-REDRESSAL</button></span>
          </a>
          ...
          >
            <ul className="flex flex-col items-center space-y-2 md:ml-auto md:flex-row md:space-y-0">
              <li className="text-gray-600 md:mr-12 hover:text-blue-600">
                <a href="/account"><button className="rounded-md border-2 border-blue-600 px-6 py-1 font-medium text-blue-600 transition-colors hover:bg-blue-600 hover:text-white">Account</button></a>
              </li>
              <li className="text-gray-600 md:mr-12 hover:text-blue-600">
                <button
